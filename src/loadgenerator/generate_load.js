...
async function logger (text) {
  ...
  const result = await exec(`k6 -e GET_ALL_RPODUCT_ENDPOINT=${GET_ALL_RPODUCT_ENDPOINT} -e SEARCH_PRODUCT_ENDPOINT=${SEARCH_PRODUCT_ENDPOINT} -e GET_ONE_PRODUCT_ENDPOINT=${GET_ONE_PRODUCT_ENDPOINT} -e SEE_AD_ENDPOINT=${SEE_AD_ENDPOINT} -e ADD_PRODUCT_TO_CART_ENDPOINT=${ADD_PRODUCT_TO_CART_ENDPOINT} -e CHECKOUT_CART_ENDPOINT=${CHECKOUT_CART_ENDPOINT} --address "localhost:8011" run script.js`)
